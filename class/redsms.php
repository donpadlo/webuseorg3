...
class Pest
{
    ...
    public function __construct($base_url)
    {
        ...
        $this->curl_opts[CURLOPT_HEADERFUNCTION] = array(
            $this,
            'handle_header'
        );
    }
...
class SMSClient
{
    ...
    public static function getSessionID_St($login, $password)
    {
        ...
        $sessionID = "";
        try {
            $sessionID = str_replace('"', '', $pest->get('/User/SessionId?login=' . $login . '&password=' . $password));
        } catch (Exception $e) {
    ...
    public static function getBalance_St($sessionID)
    {
        ...
        $balance = 0;
        try {
            $balance = str_replace('"', '', $pest->get('/User/Balance?sessionId=' . $sessionID));
        } catch (Exception $e) {
            $errorInfo = json_decode($e->getMessage());
            unset($pest);
            throw (new SMSError_Exception($errorInfo->Desc, $errorInfo->Code));
     ...
     * @param string $sessionID
     *            ID сессии. @see getSessionID_St
     * @param string $sourceAddres
     *            отправитель. До 11 латинских символов или до 15 цифровых.
     * @param string $destinationAddress
     *            адрес назначения. (Код страны+код сети+номер телефона, Пример: 79031234567
     * @param string $data
     *            Текст сообщения
     * @param mixed $sendDate
     *            дата отправки сообщения. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp. Необязательный параметр.
     * @param integer $validity
     *            Время жизни сообщения в минутах. Необязательный параметр
     *            
    ...
    public static function send_St($sessionID, $sourceAddres, $destinationAddress, $data, $sendDate = null, $validity = 0)
    {
        ...
        $result = array();
        try {
            $result = json_decode($pest->post('/Sms/Send', SMSClient::createRequestParameters($sessionID, $sourceAddres, $destinationAddress, $data, $sendDate, $validity)), true);
        } catch (Exception $e) {
            $errorInfo = json_decode($e->getMessage());
            unset($pest);
            throw (new SMSError_Exception($errorInfo->Desc, $errorInfo->Code));
     ...
     *            Текст сообщения
     * @param mixed $sendDate
     *            дата отправки сообщения по местному времени получателя. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp
     * @param integer $validity
     *            Время жизни сообщения в минутах. Необязательный параметр
     *            
     * @return array массив идентификаторов сообщений
    ...
    public static function sendByTimeZone_St($sessionID, $sourceAddres, $destinationAddress, $data, $sendDate, $validity = 0)
    {
        ...
        $result = array();
        try {
            $result = json_decode($pest->post('/Sms/SendByTimeZone', SMSClient::createRequestParameters($sessionID, $sourceAddres, $destinationAddress, $data, $sendDate, $validity)), true);
        } catch (Exception $e) {
            $errorInfo = json_decode($e->getMessage());
            unset($pest);
            throw (new SMSError_Exception($errorInfo->Desc, $errorInfo->Code));
     ...
     *            массив строк адресов назначения. (Код страны+код сети+номер телефона, Пример: 79031234567
     * @param string $data
     *            Текст сообщения
     * @param mixed $sendDate
     *            дата отправки сообщения. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp. Необязательный параметр.
     ...
     * @param string $sessionID
     *            ID сессии. @see getSessionID_St
     * @param mixed $minDateUTC
     *            начало периода выборки. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp
     * @param mixed $maxDateUTC
     *            конец периода выборки. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp
     *            
    ...
    /**
     * Конструктор.
     * В него передаются логин и пароль.
     *
     * @param string $login
     *            Логин
     * @param string $password
     ...
     *            адрес или массив адресов назначения. (Код страны+код сети+номер телефона, Пример: 79031234567
     * @param string $data
     *            Текст сообщения
     * @param mixed $sendDate
     *            дата отправки сообщения. Строка вида (YYYY-MM-DDTHH:MM:SS) или Timestamp
     * @param integer $validity
