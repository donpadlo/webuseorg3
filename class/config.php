...
class Tconfig
{
	var $mysql_host,$mysql_user,$mysql_pass,$mysql_base; // настойки соеденения с БД
	var $base_id;
  		...
  		{while ($myrow = mysqli_fetch_array($result))
  		 {
			 ...
			 $this->version=$myrow["version"];					 
                         ...
                         if (isset($_COOKIE["defaultorgid"])) {$this->defaultorgid=$_COOKIE["defaultorgid"];} else {
  		 ...
  		 } else {die('Неверный запрос Tconfig.GetConfigFromBase: ' . mysqli_error($sqlcn->idsqlconnection));}
	}
...
function SetConfigToBase()
	{
	    ...
	    smtppass='$this->smtppass',emailreplyto='$this->emailreplyto',sendemail='$this->sendemail',urlsite='$this->urlsite'";
  		...
  		if (!$result) {die('Неверный запрос Tconfig.SetToBase: ' . mysql_error());$res=FALSE;}
                return $res;
	}
}
