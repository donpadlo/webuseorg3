...
class Tequipment
{
 ...
 // уникальный идентификатор
    var $orgid;
 // какой организации принадлежит
    var $placesid;
 // в каком помещении
    var $usersid;
 // какому пользователю принадлежит
    var $nomeid;
 // связь со справочником номенклатуры
    var $tmcname;
 // наименование ТМЦ из справочника номенклатуры
    var $buhname;
 // имя по "бухгалтерии"
    var $datepost;
 // дата прихода
 ...
 // файл с фото
    var $repair;
 // в ремонте? 1 - да, 0 - нет
    var $active;
 // помечено на удаление? 1 - да, 0 - нет
    var $ip;
 // IP адрес
    var $mapx;
 // координата Х на карте
    var $mapy;
 // координата У на карте
    var $mapmoved;
 // было перемещение? 1 - да, 0 - нет
    var $mapyet;
 // отображать на карте 1 - да, 0 - нет
    ...
    function GetById($id)
    {
        ...
        $result = $sqlcn->ExecuteSQL($SQL) or die('Неверный запрос Tequipment.GetById: ' . mysqli_error($sqlcn->idsqlconnection));
