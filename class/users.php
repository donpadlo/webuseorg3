...
class Tusers
{
 ...
 // идентификатор пользователя
    var $randomid;
 // случайный идентификатор (время от времени может менятся)
    var $orgid;
 ...
 // дата и время последнего посещения
    var $active;
 // 1-не помечен на удаление
                 // далее выдергивается из профиля если оный есть по GetById
    var $fio;
 // фамилия имя отчество
    var $telephonenumber;
 // телефонный номер (сотовый)
    var $homephone;
 // телефонный номер (альтернатива)
    var $jpegphoto;
 // фотография из папки photos
    var $tab_num;
 // табельный номер
    var $post;
 // должность
    var $sslusernum;
 // номер ssl сертификата
    ...
    function UpdateLastdt($id)
    {
        ...
        $sqlcn->ExecuteSQL("UPDATE users SET lastdt='$lastdt' WHERE id='$id'") or die('Неверный запрос Tusers.UpdateLastdt: ' . mysqli_error($sqlcn->idsqlconnection));
    }
    ...
    function Update()
    {
        ...
        $sqlcn->ExecuteSQL("UPDATE users SET orgid='$this->orgid', login='$this->login'," . " `password`='$this->password', salt='$this->salt'," . " email='$this->email', mode='$this->mode'," . " active='$this->active' WHERE id='$this->id'") or die('Неверный запрос Tusers.Update (1): ' . mysqli_error($sqlcn->idsqlconnection));
        $sqlcn->ExecuteSQL("UPDATE users_profile SET fio='$this->fio'," . " telephonenumber='$this->telephonenumber'," . " homephone='$this->homephone',jpegphoto='$this->jpegphoto'," . " code='$this->tab_num',post='$this->post'" . " WHERE usersid='$this->id'") or die('Неверный запрос Tusers.Update (2): ' . mysqli_error($sqlcn->idsqlconnection));
    ...
    function GetByLogin($login)
    {
        ...
        $result = $sqlcn->ExecuteSQL("SELECT users_profile.*, users.*,
			...
			WHERE users.login ='$login'") or die('Неверный запрос Tusers.GetByLogin: ' . mysqli_error($sqlcn->idsqlconnection));
    ...
    function GetByLoginPass($login, $pass)
    {
        ...
        $result = $sqlcn->ExecuteSQL("SELECT users_profile.*, users.*,
			users.id AS sid FROM users
			INNER JOIN users_profile ON users_profile.usersid=users.id
			WHERE users.login='$login' AND users.`password`=SHA1(CONCAT(SHA1('$pass'), users.salt))") or die('Неверный запрос Tusers.GetByLoginPass: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
            $this->id = $myrow['sid'];
            $rez = $this->GetById($this->id);
            return $rez;
    ...
    function GetById($idz)
    {
        ...
        $result = $sqlcn->ExecuteSQL("SELECT users_profile.*, users.*,
			...
			WHERE users.id ='$idz'") or die('Неверный запрос Tusers.GetById: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
            $this->id = $myrow['sid'];
    ...
    function GetByRandomId($id){
        ...
        $result = $sqlcn->ExecuteSQL("SELECT users_profile.*, users.*,
			users.id AS sid FROM users
			INNER JOIN users_profile ON users_profile.usersid=users.id
			WHERE users.randomid='$id'") or die('Неверный запрос Tusers.GetByRandomId: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
            $this->id = $myrow['sid'];
            $rez = $this->GetById($this->id);
            return $rez;
    ...
    function GetByRandomIdNoProfile($id)
    {
        ...
        $rez = false;
        $id = $sqlcn->escape($id);
        $result = $sqlcn->ExecuteSQL("SELECT * FROM users WHERE randomid='$id'") or die('Неверный запрос Tusers.GetByRandomId: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
            $this->id = $myrow['id'];
            $rez = $this->GetById($this->id);
            return $rez;
    ...
    function Add($randomid, $orgid, $login, $pass, $email, $mode)
    {
        ...
        $sql = "INSERT INTO users (id, randomid, orgid, login, password, salt,
			email, mode, lastdt, active) VALUES (NULL, '$this->randomid'," . " '$this->orgid', '$this->login', " . " '$this->password', '$this->salt', " . " '$this->email', '$this->mode', NOW(), 1)";
    ...
    function GetByCode($code)
    {
        ...
        $result = $sqlcn->ExecuteSQL("SELECT users_profile.*, users.*
			...
			WHERE users_profile.code='$code'") or die('Неверный запрос Tusers.GetByCode: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
