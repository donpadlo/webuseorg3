...
function GetById($id){     // плучаем данные по идентификатору
		global $sqlcn;
  		...
  		if ($result!=''){
  		 ...
  		 } else {die('Неверный запрос Tbp_xml.GetById: ' . mysqli_error($sqlcn->idsqlconnection));}
	}
function GetLast(){     // плучаем последний созданный БП
		global $sqlcn;
...
// записываем в базу параметры о текушем шаге        
function SetNodeToBase($node){
global $sqlcn,$cfg;
$url=$cfg->urlsite."/index.php?content_page=mybp";
if (file_exists("../../../../modules/bp/$this->xml")) {$xml = simplexml_load_file("../../../../modules/bp/$this->xml");} else
if (file_exists("../../../modules/bp/$this->xml")) {$xml = simplexml_load_file("../../../modules/bp/$this->xml");} else
if (file_exists("../../modules/bp/$this->xml")) {$xml = simplexml_load_file("../../modules/bp/$this->xml");} else
if (file_exists("../modules/bp/$this->xml")) {$xml = simplexml_load_file("../modules/bp/$this->xml");} else
if (file_exists("modules/bp/$this->xml")) {$xml = simplexml_load_file("modules/bp/$this->xml");};
       //var_dump($xml);    
               foreach($xml->step as $step){                   
                   //echo "!$step->node!$node";  
                      if ($step->node==$node) {                        
                          //echo "!$step->node!$node";  
                          if ($step->thinking==""){$step->thinking=-2;};
                          if ($step->accept==""){$step->accept=-2;};
                          if ($step->cancel==""){$step->cancel=-2;};
                          if ($step->yes==""){$step->yes=-2;};
                          if ($step->no==""){$step->no=-2;};
                          if ($step->one==""){$step->one=-2;};
                          if ($step->two==""){$step->two=-2;};
                          if ($step->three==""){$step->three=-2;};
                          if ($step->four==""){$step->four=-2;};
...
global $sqlcn,$cfg;
    ...
    $url=$cfg->urlsite;    
    $result = $sqlcn->ExecuteSQL("UPDATE bp_xml SET status='$status' WHERE id='$this->id'",$cfg->base_id);
    if ($result=='') {die('Неверный запрос Tbp_xml.SetStatus: ' . mysqli_error($sqlcn->idsqlconnection));}  
    $zz=new Tusers;
