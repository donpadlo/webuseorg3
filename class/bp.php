...
class Tbp
{
 ...
 // пояснение
    var $status;
 // 0 - создано, 1-стартовано, 2-утверждено, 3-отменено, 4-отправлено на доработку
    var $node;
 // текущий узел обработки в файле xml
    var $step;
 // текущий шаг в данном БП
    var $xml;
 // схема БП файл xml
    ...
    {
        global $sqlcn;
        $result = $sqlcn->ExecuteSQL("SELECT * FROM bp_xml WHERE id='$id'") or die('Неверный запрос Tbp_xml.GetById: ' . mysqli_error($sqlcn->idsqlconnection));
    ...
    {
        global $sqlcn;
        $result = $sqlcn->ExecuteSQL('SELECT * FROM bp_xml ORDER BY id DESC LIMIT 1') or die('Неверный запрос Tbp_xml.GetById: ' . mysqli_error($sqlcn->idsqlconnection));
        while ($myrow = mysqli_fetch_array($result)) {
            $this->id = $myrow['id'];
            $this->userid = $myrow['userid'];
            $this->title = $myrow['title'];
    ...
    function SetNodeToBase($node)
    {
        ...
        } else if (file_exists("../../../modules/bp/$this->xml")) {
            $xml = simplexml_load_file("../../../modules/bp/$this->xml");
        } else if (file_exists("../../modules/bp/$this->xml")) {
            $xml = simplexml_load_file("../../modules/bp/$this->xml");
        } else if (file_exists("../modules/bp/$this->xml")) {
