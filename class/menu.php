...
/** Добавяем пункт меню. Если такой uid уже есть - то обновляем содержимое
 * 
...
function Add($parents,$name,$comment,$sort,$uid,$path){
 //если корневой ровень меню - то тупо добавляем его   
 if ($parents=="main"){
  $this->count++;         
...
function GetList($parents){
  $res=array();
 foreach ($this->arr_menu as $value) {     
  if ($parents==$value["parents"]) $res[]=$value;
 };
 array_multisort($res);
 return $res;
}    
