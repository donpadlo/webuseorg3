...
class PhoneGroups {
	...
	$result = $this->DB->queryRow($q,array("id"=>$id));
	return $result;
    }
    ...
    {
	$res = $this->DB->queryRow("SELECT * FROM sms_groups WHERE name=:name",array("name"=>$name));
	if ($res!=false) return false;
	$this->DB->insert("sms_groups",array("name"=>$name));
	return true;
    }
	...
	$result = $this->DB->queryRows($q,array("id"=>$id));
	return $result;
    }
    
    public function saveEditGroup($id,$name,$members)
    {
	$this->DB->update("sms_groups",array("name"=>$name),"id=:id",array("id"=>$id));
        $this->DB->sql("DELETE FROM sms_group_members WHERE group_id=$id");
	foreach($members as $key=>$member)
	{
	    $result[] = $member;
	    $this->DB->insert("sms_group_members",array("group_id"=>$id,"user_id"=>$member));
	}
	return $result;
    }
...
//	$groups = explode(",",$groups);
	for ($i=0; $i<count($groups); $i++)
	{
	    ...
	    $res = $this->DB->queryRows($q,array("group_name"=>$groups[$i]));
	    if (count($res)>0)
	    {
		for ($a=0;$a<count($res);$a++)
		{
		    if (!in_array($res[$a]["phone"],$result)) $result[] = $res[$a]["phone"];
		}
	    }
	}
	return $result;
