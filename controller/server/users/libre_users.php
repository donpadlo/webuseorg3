...
if ($oper == '') {
    ...
    $result = $sqlcn->ExecuteSQL("SELECT COUNT(*) AS count, org.id AS orgid," . " users.id, users.orgid, users.login, users.password, users.email," . " users.mode, users.active, org.name AS orgname" . " FROM users INNER JOIN org ON users.orgid=org.id " . $where);
    ...
    $SQL = "SELECT org.id AS orgid, users.id, users.orgid, users.login," . " users.password, users.email, users.mode, users.active," . " org.name AS orgname FROM users" . " INNER JOIN org ON users.orgid=org.id " . $where . " ORDER BY $sidx $sord LIMIT $start, $limit";
...
if ($oper == 'edit') {
    ...
    $sqlcn->ExecuteSQL($SQL) or die('Не могу обновить данные по пользователю!' . mysqli_error($sqlcn->idsqlconnection));
    exit();
}
...
if ($oper == 'add') {
    ...
    $sqlcn->ExecuteSQL($SQL) or die('Не могу добавить пользователя!' . mysqli_error($sqlcn->idsqlconnection));
    exit();
}
