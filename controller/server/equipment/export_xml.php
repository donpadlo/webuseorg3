...
if ($mode == "") {
    ...
    while ($row = mysqli_fetch_array($result)) {
	 $idd=$row["id"];
        $content_csv = $content_csv . $row["dtendgar"] . ";" . $row["tmcgo"] . ";" . $row["kntname"] . ";" . $row["grnomeid"] . ";" . $row["eqid"] . ";" . $row["ip"] . ";" . $row["eqorgid"] . ";" . $row["orgname"] . ";" . $row["vname"] . ";" . $row["grnome"] . ";" . $row["opgroup"] . ";" . $row["placesname"] . ";" . $row["fio"] . ";" . $row["nomename"] . ";" . $row["buhname"] . ";" . $row["sernum"] . ";" . $row["invnum"] . ";" . $row["shtrihkod"] . ";" . $row["datepost"] . ";" . $row["cost"] . ";" . $row["currentcost"] . ";" . $row["os"] . ";" . $row["eqmode"] . ";" . $row["eqmapyet"] . ";" . $row["eqcomment"].";";
	$dop="";
	//нахожу допполя
	$md=new Tmod;
	if ($md->IsActive("dop-pol")==1) {
	    $sql="select * from dop_pol where userid='eq$idd'";
	    $result2 = $sqlcn->ExecuteSQL($sql) or die('Не получилось выбрать список оргтехники!' . mysqli_error($sqlcn->idsqlconnection));    	 
