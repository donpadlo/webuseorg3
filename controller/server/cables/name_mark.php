...
if ($oper==''){
	if(!$sidx) $sidx =1;
        $sql="SELECT COUNT(*) AS count FROM lib_cable_name_mark";
        //echo "!$sql!";
	...
	if( $count >0 ) {
            ...
            $SQL = "SELECT * FROM lib_cable_name_mark ORDER BY $sidx $sord LIMIT $start , $limit";
            ...
            while($row = mysqli_fetch_array($result)) {
                    ...
                    $responce->rows[$i]['cell']=array($row['id'],$row['name'],$row['mark']);		
                    $i++;
            };
        };
