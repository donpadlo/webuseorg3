...
$(document).ready(function() {    
    ...
    if (window.webkitNotifications) {console.log("Уведомления поддерживаются!");}
        else { console.log("Уведомления не поддерживаются в вашей версии браузера/операционной системы.");}         
    $('<audio id="zabbix_sound"><source src="notify.ogg" type="audio/ogg"><source src="notify.mp3" type="audio/mpeg"><source src="notify.wav" type="audio/wav"></audio>').appendTo('body');
            ...
            $.get('controller/server/zabbix/getcurdashboard.php').done(
             function(data){            
                ...
                for (i in obj_for_load) { 
                  pd="success";  
                  if (obj_for_load[i]["prinum"]=="0"){pd="success";};  
                  if (obj_for_load[i]["prinum"]=="1"){pd="info";};  
                  if (obj_for_load[i]["prinum"]=="2"){pd="warning";};  
                  if (obj_for_load[i]["prinum"]=="3"){pd="error";};  
                  if (obj_for_load[i]["prinum"]=="4"){pd="error";};  
                  if (obj_for_load[i]["prinum"]=="5"){pd="error";};                    
                ...
                if (zx>0){$("#zab_img").css('border', 'red 3px solid');} else {
