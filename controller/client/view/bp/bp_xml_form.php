...
<div class="container-fluid">
	<div class="row-fluid">
		...
		<form role="form" id="myForm" ENCTYPE="multipart/form-data"
			action="controller/server/bp/bp_xml_form_addedit.php?step=<?php echo "$step&id=$bpid"; ?>"
			method="post" name="form1" target="_self">
<?php
if ($status == 0) {
    echo "<input class=form-control name=dt id=dt readonly=true value='$dt' >";
}
;
?>     
     ...
     <?php
    if ($status == 0) {
        echo "<input placeholder='Введите задачу БП' name=title id=title value='$title' class=form-control>";
    } else {
        echo "<p>$title</p>";
    }
    ;
    ...
    if ($status == 0) {
     ...
     <textarea placeholder='Подробное описание задачи'
				class=form-control name=bodytxt><?php echo "$bodytxt";?></textarea>
     <?php
        ...
        if ($status == 0) {
         ...
         <?php
            ...
            <select class="form-control" name="status" id="status"
				...
				<option value=3 <?php if ($status=="3"){echo "selected";};?>>Отменен</option>
				<option value=4 <?php if ($status=="4"){echo "selected";};?>>В
					доработке</option>
			</select> <label>Схема БП:</label>
         <?php
        }
