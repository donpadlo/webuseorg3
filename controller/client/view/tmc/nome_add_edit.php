...
if (($user->mode == 1) or ($user->TestRoles('1,3'))) {
    ...
    if ($step == "edit") {
        ...
        if ($result != '') {
            ...
            die('Неверный запрос : ' . mysqli_error($sqlcn->idsqlconnection));
        }
    }
    ;
    ?>
...
<div class="container-fluid">
	<div class="row">
		...
		<form role="form" id='myForm' ENCTYPE="multipart/form-data"
			action="controller/server/tmc/add_edit_tmc.php?step=<?php echo "$step&id=$id"; ?>"
			method="post" name="form1" target="_self">
			<div class="row-fluid">
				<div class="col-xs-6 col-md-6 col-sm-6">
					<label for="groupid">Группа</label></br> <select
						class='chosen-select' class="form-control" name=groupid
						id="groupid">
    <?php
    ...
    ?>   
				...
				<div class="col-xs-6 col-md-6 col-sm-6">
					<label for="vendorid">Производитель</label></br> <select
						class='chosen-select' class="form-control" name=vendorid
						id="vendorid">
    <?php
    ...
    ?>   
        ...
        for (var selector in config) {
			...
			<div class="form-group">
				<label for="namenome">Наименование</label> <input
					class="form-control"
					placeholder="Введите наименование номенклатуры" name="namenome"
					id="namenome" size=100 value="<?php echo "$name";?>">
			</div>    
<?php
    $view = false;
    if ($step == "edit") {
        ...
        ;
    } else if (($user->mode == 1) or ($user->TestRoles('1,4'))) {
        $view = true;
    }
    ;
    if ($view == true) {
