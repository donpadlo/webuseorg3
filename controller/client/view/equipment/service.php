...
if ($step=='edit'){
    ...
    $result = $sqlcn->ExecuteSQL($SQL)  or die("Не получилось выбрать список ремонтов!".mysqli_error($sqlcn->idsqlconnection));
     while($row = mysqli_fetch_array($result)) {
                $kntid=$row['kntid'];
                $cost=$row['cost'];
                $dtpost=MySQLDateTimeToDateTimeNoTime($row["dt"]);echo "<script>dtpost='$dtpost'</script>";
                $dt=MySQLDateTimeToDateTimeNoTime($row["dtend"]);echo "<script>dt='$dt';step='edit';</script>";                
                $comment=$row['comment'];
                $status=$row['status'];
                ...
                $dt="";echo "<script>dt='$dt';step='add';</script>";                
                $comment="";
                $status="1";
                $userfrom="-1";
                $userto="-1";
        ...
        <?php
           $SQL = "SELECT users.id, users.login, users_profile.fio FROM users INNER JOIN users_profile ON users.id = users_profile.usersid WHERE users.active=1 ORDER BY users.login";
            ...
            $sts="<select class='chosen-select'name=suserid1 id=suserid1>";
                ...
                while($row = mysqli_fetch_array($result)) {
                 $sl='';
                 if ($row["id"]==$userfrom){$sl='selected';};	                    
