			...
			<div class="row-fluid">
				<div class="col-xs-6 col-md-6 col-sm-6">
					<div class="form-group">
						...
						<input class="form-control" name=alert_max_count
							id=alert_max_count value="<?php echo "$alert_max_count"; ?>"> <label
							for="alert_max_time">Период (в сек.), после которого скрипт будет
							считаться не отработавшим:</label> <input class="form-control"
							name=alert_max_time id=alert_max_time
							value="<?php echo "$alert_max_time"; ?>"> <input type="hidden"
							name=sid id=sid value="<?php echo "$sid"; ?>">
					</div>
				</div>
				<div class="col-xs-6 col-md-6 col-sm-6">
					<div class="form-group">
						...
						<label for="sms_group_id">Получатели:</label> <select
							class='chosen-select' name=sms_group_id id=sms_group_id>
			<?php
$sql = "select * from sms_groups";
$result = $sqlcn->ExecuteSQL($sql);
while ($row = mysqli_fetch_array($result)) {
    echo "<option value=" . $row["id"];
    if ($row['id'] == $sms_group_id) {
        echo " selected";
    }
    ;
    $nm = $row['name'];
