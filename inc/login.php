...
if (isset($_GET["login_step"])) {
    ...
    if ($_GET["login_step"]=='enter'){ // если вход то пытаемся зайти
        $enter_user_login=$_POST["enter_user_login"];if ($enter_user_login==""){$err[]="Логин не может быть пустым!";}
        $enter_user_pass=$_POST["enter_user_pass"];  if ($enter_user_pass==""){$err[]="Пароль не может быть пустым!";}      
